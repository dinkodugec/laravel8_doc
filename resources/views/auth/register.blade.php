...
@section('content')
  <form method="POST" action="{{ route('register') }}">
    ...
    <div class="form-group">
      <label>Name</label>
      <input name="name" value="{{ old('name') }}" required class="form-control{{ $errors->has('name') ? ' is-invalid': '' }}">  {{-- is invalid is bootrstrap elemen --}}t
      @if ($errors->has('name'))
      <span class="invalid-feedback">{{--  class from bootstrap --}}
        <strong>{{ $errors->first('name') }}</strong>  {{-- first error display --}}
      </span>
    @endif
    </div>
